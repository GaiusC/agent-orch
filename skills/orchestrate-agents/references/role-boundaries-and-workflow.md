...
- Route implementation and repair through `auto` (recommended), `cc-exec`/`cc-continue`, or `agy-exec`/`agy-continue`.
...
4. The coordinator launches the implementation worker (via `auto`, `cc-exec`, or `agy-exec`).
5. The accepter inspects worker evidence and diff.
6. The coordinator launches AGY for required investigation or verification gates.
7. The accepter accepts, rejects, or sends same-session delta feedback to the worker.
8. The coordinator applies only accepted patches, then records/generated handoff state and remaining contracts.
